...
def clffactory(clfname, **kwargs):
    ...
    if ('svm' in clfname) or ('svc' in clfname):
        ...
        clf = SVC(kernel=k, gamma=g, decision_function_shape=ov)
    ...
    elif 'lsvm' in clfname:
        k = kwargs['kernel'] if 'kernel' in kwargs else 'linear'
        ov = 'ovr'
        clf = SVC(kernel=k, decision_function_shape=ov)
    elif 'knn' in clfname:
